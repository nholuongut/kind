...
import (
	...
	"https://github.com/nholuongut/kind/pkg/exec"

	"https://github.com/nholuongut/kind/pkg/cluster/internal/loadbalancer"
	"https://github.com/nholuongut/kind/pkg/cluster/internal/providers/common"
...
func commonArgs(cfg *config.Cluster, networkName string, nodeNames []string) ([]string, error) {
	...
	// https://github.com/nholuongut/kind/issues/1416#issuecomment-606514724
	if mountDevMapper() {
		args = append(args, "--volume", "/dev/mapper:/dev/mapper")
	}

	// rootless: use fuse-overlayfs by default
	// https://github.com/nholuongut/kind/issues/2275
	if mountFuse() {
...
func getProxyEnv(cfg *config.Cluster, networkName string, nodeNames []string) (map[string]string, error) {
	...
	if len(envs) > 0 {
		...
		// https://github.com/kubernetes/dns/blob/main/docs/specification.md
...
func generatePortMappings(clusterIPFamily config.ClusterIPFamily, portMappings ...config.PortMapping) ([]string, error) {
	...
	for _, pm := range portMappings {
		...
		// https://github.com/containers/libpod/blob/main/pkg/spec/ports.go#L68-L69
